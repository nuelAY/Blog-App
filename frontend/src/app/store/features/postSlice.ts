...
export const fetchBlogById = createAsyncThunk('blogs/fetchById', async (id: string, thunkAPI) => {
  try {
...
export const updateBlog = createAsyncThunk('blogs/update', async ({ id, data }: { id: string; data: Partial<Blog> }, thunkAPI) => {
  try {
    const state = thunkAPI.getState() as RootState;
    const token = state.auth.user?.token;
